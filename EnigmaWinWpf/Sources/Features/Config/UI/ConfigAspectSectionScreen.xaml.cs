// ConfigAspectSectionScreen.xaml.cs
...
public partial class ConfigAspectSectionScreen : UserControl
{
    private ConfigAspectSectionViewModel? _vm;
    ...
    public ConfigAspectSectionScreen()
    {
        InitializeComponent();
    ...
    {
        _vm = DataContext as ConfigAspectSectionViewModel;
    }
    ...
    private void OnHelpClicked(object sender, RoutedEventArgs e)
    {
        ...
        {
            Title = _vm.LabelHelpTitle, Width = 500, Height = 300,
    ...
    {
        if (sender is not Button { DataContext: AspectRowViewModel row }) return;

        var selectedColor = row.CurrentColor;

        var preview = new Border
        {
            Height = 44, CornerRadius = new CornerRadius(6),
        ...
        void SelectColor(Color c, Border? swatchBorder)
        {
            selectedColor = c;
            ...
            if (swatchBorder is not null)
            {
                ...
                swatchBorder.BorderBrush = Brushes.Black;
            }
        }

        var palettePanel = new StackPanel { Margin = new Thickness(0, 0, 0, 2) };
        for (int rowIdx = 0; rowIdx < 6; rowIdx++)
        {
            var rowPanel = new StackPanel { Orientation = Orientation.Horizontal };
            for (int colIdx = 0; colIdx < 8; colIdx++)
            {
                var paletteColor = (Color)ColorConverter.ConvertFromString(ColorPalette[rowIdx * 8 + colIdx]);
                var swatchBorder = new Border
                {
                    Width = 28, Height = 28, CornerRadius = new CornerRadius(3),
                    Background = new SolidColorBrush(paletteColor), Margin = new Thickness(1)
                };
        ...
        var cancelBtn = new Button { Content = "Annuleer", Margin = new Thickness(0, 0, 8, 0) };
        ...
        var dialog = new Window
        {
            Title = row.AspectName, Width = 310,
