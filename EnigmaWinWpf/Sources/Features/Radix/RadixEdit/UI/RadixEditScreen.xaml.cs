...
public partial class RadixEditScreen : UserControl
{
    ...
    {
        if (_isUpdatingExpanders || sender is not Expander expanded || _vm is null) return;
    ...
    {
        if (sender is not Expander section || _isUpdatingExpanders || section.IsExpanded) return;
