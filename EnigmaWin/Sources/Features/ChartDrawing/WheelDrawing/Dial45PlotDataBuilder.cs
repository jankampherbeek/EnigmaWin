...
/// <summary>
/// Builds WheelPlotData for the 45° dial wheel.
/// Each factor is mapped to (longitude mod 45) × 8, so the full dial range
/// represents one eighth of the zodiac compressed into 360 visual degrees.
/// </summary>
public static class Dial45PlotDataBuilder
{
    public static WheelPlotData Build(FullChart chart, UserConfiguration? config = null)
    {
        ...
        foreach (var (factor, position) in chart.Coordinates)
        {
            ...
            var dialAngle = Dial45Angle(eclPos);
            var text      = Dial45PositionText(eclPos, speedType);
        ...
        var ascDialAngle = Dial45Angle(ascLong);
        items.Add(new WheelPlotItem(
            ...
            PositionText:      Dial45PositionText(ascLong),
            SpeedType:         SpeedType.Direct));

        var mcDialAngle = Dial45Angle(mcLong);
        items.Add(new WheelPlotItem(
            Factor:            Factors.Mc,
            ...
            PositionText:      Dial45PositionText(mcLong),
            SpeedType:         SpeedType.Direct));
