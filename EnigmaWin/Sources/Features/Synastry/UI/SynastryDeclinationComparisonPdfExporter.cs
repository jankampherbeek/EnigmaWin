// SynastryDeclinationComparisonPdfExporter.cs
...
/// <summary>Column labels for the synastry declination comparison PDF export header rows.</summary>
public sealed record SynastryDeclinationComparisonPdfLabels(
    string Title, string ChartAName, string ChartBName, string ColOrb);

/// <summary>
/// Builds the Synastry declination comparison PDF as real vector text, mirroring
/// SynastryAspectComparisonPdfExporter's two-tables-side-by-side layout.
/// </summary>
public static class SynastryDeclinationComparisonPdfExporter
{
    ...
    static SynastryDeclinationComparisonPdfExporter()
    {
        if (GlobalFontSettings.FontResolver is null)
            GlobalFontSettings.FontResolver = new EmbeddedGlyphFontResolver();
    }

    public static void Export(string filePath, SynastryDeclinationComparisonPdfLabels labels,
                               IReadOnlyList<SynastryDeclinationRow> rowsFromA, IReadOnlyList<SynastryDeclinationRow> rowsFromB)
    {
    ...
    private static void DrawRow(XGraphics gfx, double x, double y, SynastryDeclinationRow row, XFont glyphFont, XFont monoFont)
    {
        var cx = x;
        gfx.DrawString(row.RadixGlyph, glyphFont, XBrushes.Black, new XPoint(cx, y + GlyphSize - RowSize));
        cx += ColGlyph;
        gfx.DrawString(row.MatchGlyph, glyphFont, XBrushes.Black, new XPoint(cx, y + GlyphSize - RowSize));
        cx += ColGlyph;
