...
public partial class SolarFactsheetWindow : Window
{
    public SolarFactsheetWindow(string title)
    {
        Title = title;
        InitializeComponent();
    ...
    private void LoadPdf()
    {
        ...
        var pdfPath  = Path.Combine(
            ...
            "Resources", "FactSheets", $"solar_{fileCode}.pdf");

        if (!File.Exists(pdfPath))
            pdfPath = Path.Combine(
                AppDomain.CurrentDomain.BaseDirectory,
                "Resources", "FactSheets", "solar_en.pdf");
