// ConfigTransitsSectionViewModel.cs
...
public sealed partial class ConfigTransitsSectionViewModel : ObservableObject
{
    ...
    public string SectionTitle        => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.prog.nav.transits");
    ...
    public string LabelHelpTooltip    => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.prog.transits.help.tooltip");
    public string LabelHelpTitle      => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.prog.transits.help.groupbox");
    public string LabelHelpClose      => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.help.close");
    public string LabelHelpLine1      => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.prog.transits.help.line1");
    public string LabelHelpLine2      => _rosetta.GetText(RbFile.ConfigEdit, "view.configedit.prog.transits.help.line2");

    // ── Constructor ──────────────────────────────────────────────────────────

    public ConfigTransitsSectionViewModel(
    ...
    internal async Task SaveAsync()
    {
        ...
        var newTransits = new TransitsConfig(
            FactorRows.Where(r => r.IsSelected).Select(r => r.Factor).ToList(),
            FromSexagesimal(OrbDeg, OrbMin));

        config.ProgressionsConfig = config.ProgressionsConfig with { Transits = newTransits };
    ...
    private TransitsConfig CurrentConfig() =>
        _configContext.EditingConfig?.ProgressionsConfig.Transits ?? TransitsConfig.Default;
